...
class CodeInputTextField extends StatefulWidget {
  ...
  CodeInputTextField({Key? key, 
    ...
    this.countdown = 60,
    this.getSmsCode,
  }) : inputFormatters = inputFormatter ?? <TextInputFormatter>[FilteringTextInputFormatter.digitsOnly], super(key: key);
...
class _CodePaint extends CustomPainter {
  ...
  _CodePaint({
    ...
    this.space = 4.0,
    required this.cursorAlpha,
    required this.cursorAnimationListener,
  }) {
    this.text = text.trim();
  }
  ...
  bool shouldRepaint(CustomPainter oldDelegate) =>
      !(oldDelegate is _CodePaint && oldDelegate.text == text);
  ...
  _drawUnderLine(Canvas canvas, Size size) {
    ...
    if (text.length < codeLength) {
      Color cursorColor = const Color(0xff10BFC7);
